...
The import-graph rules (<RuleRef id="B1" />, <RuleRef id="C3" />, <RuleRef id="C4" />, <RuleRef id="C5" />, <RuleRef id="C6" />) currently require a resolvable JS/TS import graph. On other stacks these rules are reported as **not evaluated** rather than silently passing, and scan confidence is reduced. See [Scoring non-JS repositories](/docs/cli/non-js-repos) for what a partial analysis covers and how to raise confidence with a manifest.
