...
- Line-count rules (<RuleRef id="C2" />, <RuleRef id="E1" />) — line counting covers `.py`, `.go`, `.rs`, `.rb`, `.java`, and more

Five rules require a resolvable **JS/TS import graph** and are reported as **not evaluated** on other stacks:

| Rule | What it needs |
|------|---------------|
| <RuleRef id="B1" /> — circular imports | import graph |
| <RuleRef id="C3" /> — import surface per file | import graph |
| <RuleRef id="C4" /> — high fan-in documentation | import graph |
| <RuleRef id="C5" /> — import chain depth | import graph |
| <RuleRef id="C6" /> — boundary surface in entry files | JS/TS entry-file analysis |
