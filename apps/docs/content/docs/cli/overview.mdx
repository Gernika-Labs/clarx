...
The package installs a `clarx` binary.
...
In a terminal, `clarx score` opens the [interactive TUI](/docs/cli/tui) by default. Use `--ui text` for a plain readline prompt, or `--format json|markdown|sarif` for machine-readable output.
...
In the TUI, press `Enter` on an issue to open a formatted detail view with the same guidance.
