...
clarx score --ui text          # force legacy text UI instead of Ink
clarx init                     # generate starter manifest
clarx explain C2               # explain a rule
```

Interactive `clarx score` runs use the Ink UI by default when stdout is a TTY.
Use `--ui text` to force the text interface. Non-TTY and machine-readable runs
