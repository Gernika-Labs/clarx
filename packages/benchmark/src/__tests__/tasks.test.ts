...
describe('task validation', () => {
  it('accepts a well-formed task', () => {
    expect(validateTask(task(), ROOT, MANIFEST)).toEqual([])
  })
  ...
  it('rejects a prompt that names Clarx vocabulary', () => {
    ...
    const violations = validateTask(task({ prompt: 'Update the manifest so the converters agree about separators at the start of a string.' }), ROOT, MANIFEST)
  ...
  it('rejects a task with no objective check', () => {
    const violations = validateTask(task({ checkCommand: null, rubric: null }), ROOT, MANIFEST)
    expect(violations.map(v => v.rule)).toContain('no-objective-check')
  })

  it('rejects a task with both a command and a rubric', () => {
    const violations = validateTask(task({ rubric: ['does the thing'] }), ROOT, MANIFEST)
  ...
  it('rejects duplicate task ids', () => {
    const violations = validateSuite([task(), task()], ROOT, MANIFEST)
